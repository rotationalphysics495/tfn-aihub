...
Plant managers can now interact with the AI-generated morning report summary instead of just reading it. An "Ask about this" button opens the AI chat pre-loaded with today's report context, asset names in the summary are clickable links that jump to the relevant action item, and suggested follow-up questions appear as clickable chips so managers know what to ask without starting from scratch.

### Who Should Test

A Plant Manager or Operations Lead who regularly uses the morning report. No technical knowledge is needed — just familiarity with the morning report page and the AI chat feature.
...
3. **Test Data Setup**
   - Ensure the morning report for at least one recent date has:
     ...
     - At least 3 action items across different categories (e.g., safety, OEE, financial)
...
| 2 | Look below the AI summary text, near "Powered by AI analysis" | An "Ask about this" button with a chat icon is visible |
...
| 3 | Type a follow-up question related to the report, e.g., "What was the root cause for [an asset name from the summary]?" | The AI responds with information that references specific data from today's morning report — not a generic answer |
...
| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | Type a question unrelated to the morning report, e.g., "What is the current status of asset [name]?" or "Show me last month's production data" | The AI responds with a relevant answer, using its normal data tools — not limited to only morning report context |
...
| 3 | Observe the target action item card | The card briefly highlights (a blue ring/flash) to draw attention to it |
...
| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | Look below the "Ask about this" button | 2-3 suggested question chips are visible (e.g., "What were the top downtime reasons for Grinder 5?", "How does today's OEE compare to last week?") |
| 2 | Verify the questions are relevant | The questions reference asset names, categories, or metrics from today's report — they are not generic |
| 3 | Verify the chips have arrow icons | Each chip has a small arrow icon indicating it is clickable |
...
### Button Hidden When Summary Is Loading or Unavailable
...
| 2 | Navigate to a different report date that has different action items | The suggested question chips update to reflect the new report's content (different asset names, categories) |
| 3 | Verify the new suggestions are relevant | The questions reference data from the newly loaded report, not the previous one |

**Result**: ☐ Pass  ☐ Fail

---

### Clear Context in Chat
...
| 4 | Ask a question | The AI responds using its standard tools, without referencing the morning report context |
...
- [ ] Click asset names in the summary to scroll to the matching action item
